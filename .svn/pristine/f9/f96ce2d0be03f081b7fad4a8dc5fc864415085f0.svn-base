...
/*.bigdrop{
	...
	<form method="post" action="{{URL::to('/') }}/admin/salesBill/{{$view_order->sales_id}}" data-parsley-validate class="m-t-40" onsubmit="return validateForm()">
		{!! csrf_field() !!} 
		{{ method_field('PUT') }}
		...
		<div class="container-fluid" style="margin-top:-30px">
			<div class="row">
				...
				<div class="col-md-9">
					<div class="card card-outline-info">
						<div class="card-header">
							<h4 class="m-b-0 text-white">Bill Details (Exchange Product)</h4>
						</div>
						<div class="card-block">
							<div class="row">
					    ...
					    <table class="table" id="stock_request_send" style="width: 100%">
					        ...
					        <tbody>
					            ...
					            <tr>
					                ...
					                <td>{{ $details->mrp }}</td>
					                <td><a onclick="(exchange_product({{ $details->sales_ids}}));" style="cursor: pointer;"><img src="{{URL::to('/') }}/img/delete.png"></a></td>
									...
									<div class="form-group">
										...
										<div class="col-md-12">
											<select class="form-control p-0" required  data-parsley-required-message="Select Mode" id="payment_type" name="payment_type" onclick="check_mode()">
												...
												<option id="normal" value="normal" @if($count>0 && $pay_detail->status=='normal'){{ 'selected' }}@endif >Normal</option>
								...
								<div class="col-md-2" id="cash_paid_div" style="display: none">
									<div class="form-group">
										...
										<div class="col-md-12">
											...
											<input type="hidden"   name="balance_amount" id="balance_amount" value="{{ $total_balance_amount or 0 }}">
											<input type="hidden"   name="total_paid_amount" id="total_paid_amount" value="{{ $total_paid_amount or 0 }}">  
											<input type="hidden"   name="paid_amount1" id="paid_amount1" value="{{ $pay_detail->paid_amount or 0 }}">
...
<script type="text/javascript">
	// On submit validasion
function validateForm() 
{
    var total_amount = $("#total_amount").val();
    if (total_amount == 0)
     {
 		swal({title: "Add Product!",text: "Add Product.",type: "info", timer: 3000});
 		return false;
...
$('.chose').on('select2:select', function (e) {		
	...
	function exchange_product(id) 
	{
        ...
        if (isConfirm) {
        $.ajax({
            url: "{{ URL::to('admin/exchangeBill/exchange_product') }}",
            type: "get",
            ...
            success: function (data) {
                ...
                setTimeout(function() {
              window.location.href = "{{URL::to('/')}}/admin/exchange_product/"+sales_id
            }, 2000);
