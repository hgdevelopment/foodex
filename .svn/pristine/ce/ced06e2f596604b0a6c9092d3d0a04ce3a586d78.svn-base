...
class StockTransferController extends Controller
{
    ...
    public function stock_request_confirm($id)
    {
      ...
      $data=Transfer_request::where('transfer_request.id',$id);
       ...
       $data_products=DB::table('transfer_products')->where('transfer_products.transfer_request_id',$id)
    	->leftJoin('products','products.product_number','=','transfer_products.product_code')->where('products.added_branch',$branch)->select('transfer_products.*','products.*','products.id as productId')->get();
    ...
    public function stock_request_store(Request $request){
      ...
      if($id==null){
       ...
       for ($i=0; $i<count($request->product); $i++ ) {
          ...
          $type='Stock Request';
            $report='Add New Stock Request.To: '. $request->to_branch;
    ...
    public function stock_confirm(Request $request)
    {
        ...
        $type='Stock Request';
        $report='Stock Request Confirm  .Request Id: '. $request->product_request_id;
        Controller::logReport($type,$report);

    	  return $request->product_request_id;
       }

    public function transferStockPrint($id)
    {
      $branch_id = Auth::guard('admin')->user()->branch;
      $transfer_product_details=Transfer_confirm_product::join('products','products.product_number','=','Transfer_confirm_products.product_code')
         ->where("Transfer_confirm_products.to_branch",$branch_id)
          ->where("products.added_branch",$branch_id)
         ->where("Transfer_confirm_products.transfer_request_id",$id)
         ->select('Transfer_confirm_products.*','products.*')->get();

        $address = branch::where('id',$branch_id)->first();

        foreach($transfer_product_details as $details)
        {
          $transfer_to = $details->from_branch;
          $transfer_date = $details->created_at;
        }
        $branch_name = branch::where('branches.id',$transfer_to)->first();
       return view('admin.transfer.transferStockPrint',compact('transfer_product_details','address','branch_name','transfer_date')); 
    }
    ...
    public function stock_confirm_instock(Request $request){
      ...
      //query stock
        ...
        $type='Stock Request';
        $report='Transfer Confirm product Add In Purchage Product .Product Name: '. $product->product_name;
        Controller::logReport($type,$report);
    ...
    public function get_batch_qtyAvl(Request $request){
      $data=Purchase_product::join('purchase_orders','purchase_orders.id','=','purchase_products.order_id')->where('purchase_products.product_id',$request->product_id)->select('purchase_orders.id','purchase_orders.purchase_no')->get();
      //$data=Purchase_order::where('branch_id',Auth::guard('admin')->user()->branch)->get();
      $_make_array=[];
