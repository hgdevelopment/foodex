												...
												<td>
													<select name="batchno[]" id="batchno" class="form-control batchno" style="width:100%;">
														@foreach($purchase as $purchases)
															<option value='{{ $purchases->id }}'>{{ $purchases->purchase_no }}</option>
														@endforeach
													</select>
												</td>
												<td>
					...
					<div class="table-responsive m-t-40">
						<table id="example23" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
							...
							<tbody>
								...
								<tr>
									<td>{{$sl++}}</td>
									<td>{{$products->branch_id}}</td>
									<td>{{$products->purchase_no}}</td>
...
<script>
	...
	.on('click', '.add-more', function() {
		...
		$clone.find('select[name="product[]"]').select2({
			...
			ajax: {
				url: '{{URL::to('/')}}/admin/damageproduct/damaged_product_select',
				dataType: 'json',
				method:'post',
	...
	.on('success.form.fv', function(e) {
		...
		$.ajax({
			url: '{{URL::to('/') }}/admin/damageproduct/damaged_product',
			method: 'POST',
			data: data,
