		...
		<div class="container" style="max-width: 500px;">
				...
				<div class="col-md-12">
					...
					<div class="row">
						<div class="invoice-title" style="text-align: right; margin-right:15px;">
							<address>
								Transfer Date : {{ $transfer_date->format('d-m-Y   H:i:s ') }}<br>
								Stock Transfer To: {{ $branch_name->branch_name }}
							</address>
						</div>
					</div>
				</div>
				...
				<table class="table" style="max-width: 500px;font-size: 14px !important " cellpadding="0" cellspacing="0">
					...
					<tbody>
						...
						@foreach($transfer_product_details as $details)
							...
							<tr>
								<td style="width:60%;font-size: 13.5px !important">{{ $details->product_name}}</td>
								<td class="text-right" valign="center" style="width:10%;padding: 2px 0px !important">{{ $details->product_qty }}</td>
								<td class="text-right" valign="center"  style="width:10%;padding: 2px 2px !important"">{{ $details->billing_price }}</td>
								<td class="text-right" valign="center"  style="width:10%;padding: 2px 2px !important"">{{ $mrp }}</td>
							</tr>
						@endforeach
					</tbody>
				</table>
			...
			<div class="row" style="    margin-right: -30px !important;">
				<table width="100%" style="font-size: 14px !important " cellpadding="0" cellspacing="0">
					<tbody>
						<tr>
							<td align="right" colspan="2">Total Products :{{ $product_count }} Products </td>
						</tr>
						...
						<tr>
							<td align="right">Total Amount </td><td> :</td><td>{{ $total_amount }}</td>
						</tr>
					</tbody>
				</table>
			</div>
